...
public class RoomDAOImpl extends GenericsDAOImpl<Room> implements RoomDAO {
    ...
    @Override
    public List<Room> findAcRoom(List<Room> rooms, String acType) {
        return rooms.stream ()
                .filter (room -> room.getAcType ().equalsIgnoreCase (acType))
                .toList ();
    }
    
    @Override
    public List<Room> findCheaperRoom (List<Room> rooms,BigDecimal maxPrice) {
        return rooms.stream()
                .filter(room -> room.getPrice().compareTo(maxPrice) <= 0) // ✅ price <= 3000
                .collect (Collectors.toList ());
        
    }
    
    @Override
    public List<Room> findBedRoom (List<Room> rooms,String bed) {
        return rooms.stream ()
                .filter (room -> room.getBed ().equalsIgnoreCase (bed))
                .toList ();
    }
    
    @Override
    public Room findRoomByRooNo (int roomNo) {
        try (Session session = sessionFactory.openSession ()) {
            return session.createQuery ("FROM Room WHERE roomNumber = :roomNo", Room.class)
                    .setParameter("roomNumber", roomNo)
                    .uniqueResult();
        }
    }
}
